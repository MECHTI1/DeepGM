...
def pocket_lining_features(struc_dir, site, this_protein, res_nums):
    try:
        ...
        # If pocket fails, don't kill the whole site: return empty pocket features
        pocket_shape, pocket_lining, pocket_info = get_empty_pocket_lining_features()
        # optional: keep a warning
        print(f"[WARN] pocket features failed in {struc_dir} for {site.SITE_ID}: {type(e).__name__}: {e}", flush=True)

    pocket_shape = pocket_shape.T.add_prefix("pkt_shape_").T
    pocket_lining = pocket_lining.T.add_prefix("pkt_lining_").T
