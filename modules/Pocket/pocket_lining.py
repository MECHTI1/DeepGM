...
def get_nearby_grid(pocket_list, cluster_prediction, SITE_center, cutoff):
    ...
    pocket_list = pocket_list[ np.isin(cluster_prediction, metal_cluster)]
    return(pocket_list)
...
def split_pocket_lining(pocket_lining_list, pocket_points, myProtein, cutoff = 2.2):
    ...
    for entry in pocket_lining_list:
        ...
        this_dist = scipy.spatial.distance.cdist(this_res.Coords[np.where(np.isin(this_res.Atoms, keep_atoms))], pocket_points) #distance between all non-H coords of residue and the pocket grid points
